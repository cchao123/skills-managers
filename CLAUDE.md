...
Skills Manager is a desktop application for managing Claude Code skills and plugins. It's built with **Tauri 2** (Rust backend) and **React + TypeScript** (Vite frontend), targeting macOS and Windows.

The application scans Claude Code's plugin directories, displays skills in a UI, and enables/disables them by modifying `~/.claude/settings.json`. It also supports cloning GitHub repositories containing custom skills.
...
|--------|------|----------------|
| **SkillScanner** | scanner.rs | Scans `~/.claude/plugins/cache/` and `~/.claude/skills-manager/` for skills, parses SKILL.md frontmatter |
| **SettingsManager** | settings.rs | Reads/writes `~/.claude/settings.json`, manages `enabledPlugins` field |
...
1. **Scanning**: Reads `~/.claude/plugins/cache/[marketplace]/[plugin]/[version]/skills/` for `SKILL.md` files
2. **Enabling/Disabling**: Modifies `~/.claude/settings.json`:
   ```json
   {
     "enabledPlugins": {
   ...
   ```
3. **GitHub Skills**: Clones repos to `~/.claude/skills-manager/[repo-name]/skills/`
...
| GitHub skills | `~/.claude/skills-manager/[repo-name]/skills/` |
| App config | `~/.claude/plugins/data/skills-manager/github-config.json` |
